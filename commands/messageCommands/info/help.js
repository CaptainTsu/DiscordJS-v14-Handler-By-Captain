    ...
    async execute(message, args) {
      ...
      const resetCode = textColors.styles.reset;
        ...
        fs.readdirSync(commandsPath).forEach(category => {
            ...
            if (fs.lstatSync(categoryPath).isDirectory()) { // Check if it's a directory
        ...
        if (args.length > 0) {
            ...
            const foundCommand = Object.values(categories).flat().find(cmd => cmd.name === commandName);
...
\`\`\``;
        ...
        const sentMessage = await message.reply({ embeds: [homeEmbed], components: [row] });
